...
class MongoTest(unittest.TestCase):
    ...
    def test_get_item(self):
        ...
        """
        users = self.mongoDataManager.load_items(self.user_collection, "Date", 2015)
        self.assertTrue(len(users) != 0)
        users = self.mongoDataManager.load_items(self.user_collection, "Date", 2016)
        self.assertTrue(len(users) == 0)
    ...
    def test_index(self):
        ...
        objects = self.mongoDataManager.load_items(self.user_collection, "Date", 2015)
        if len(objects) == 0:
            self.mongoDataManager.save_items(self.user_collection, test_obj)
